...
### Fidelity (CSV positions import)
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/fidelity/import` | Body `{csv, commit}` — parse a Fidelity "Portfolio Positions" CSV. `commit=false` previews the matched/skipped breakdown; `commit=true` full-replaces the matched accounts' holdings. Rows match existing Plaid-linked accounts by last-4 mask; unmatched accounts (e.g. the 401k umbrella) are skipped with a reason, never auto-created |
...
| **v2h — Fidelity CSV positions import** | ✅ Shipped | Plaid only exposes Fidelity *balances*, so per-position detail comes from Fidelity's "Portfolio Positions" CSV. An import panel on `/dashboard/holdings` previews then full-replaces the matched accounts' holdings: parses stocks / ETFs / **options** (Fidelity's `-BMNR280121C65` → OCC), matches accounts by last-4 mask, back-derives price from value when Last Price is blank, and **skips the 401(k) "umbrella" lump to avoid double-counting** the BrokerageLink sleeves. Imported tickers then price live via Yahoo. **Re-running Plaid Sync reverts Fidelity to balances — re-import the CSV instead** |
